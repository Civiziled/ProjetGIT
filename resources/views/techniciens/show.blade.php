...
@section('content')
    <div class="mb-8">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-3xl font-bold text-gray-900">{{ $technicien->name }}</h1>
                <p class="text-gray-600 mt-2">Profil technicien et historique des interventions</p>
            </div>
            <div class="flex space-x-3">
                @can('update', $technicien)
                    <a href="{{ route('techniciens.edit', $technicien) }}" class="bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700 transition duration-300">
                        Modifier
                    </a>
                @endcan
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-1">
            <div class="bg-white rounded-lg shadow p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Informations technicien</h3>
                
                <div class="space-y-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nom complet</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $technicien->name }}</p>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Email</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $technicien->email }}</p>
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Téléphone</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $technicien->phone }}</p>
                    </div>

                    @if($technicien->address)
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Adresse</label>
                            <p class="mt-1 text-sm text-gray-900">{{ $technicien->address }}</p>
                        </div>
                    @endif

                    @if($technicien->specialisation)
                        <div>
                            <label class="block text-sm font-medium text-gray-700">Spécialisation</label>
                            <p class="mt-1 text-sm text-gray-900">{{ $technicien->specialisation }}</p>
                        </div>
                    @endif
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Technicien depuis</label>
                        <p class="mt-1 text-sm text-gray-900">{{ $technicien->created_at->format('d/m/Y') }}</p>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-6 border-t border-gray-200">
                    <div class="flex justify-between items-center">
                        ...
                        <span class="text-sm font-bold text-gray-900">{{ $technicien->interventions->count() }}</span>
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="p-6">
                                ...
                                <div class="border rounded-lg p-4 hover:bg-gray-50">
                                    <div class="flex justify-between items-start">
                                        ...
                                        <div class="ml-4">
                                            ...
                                            <a href="{{ route('interventions.show', ['intervention' => $intervention->id, 'from' => 'technicien']) }}">
                                                Voir l’intervention
                                            </a>
                                        </div>
                                    </div>
                    ...
                    @else
                        <div class="text-center py-8">
                            ...
                            <p class="mt-1 text-sm text-gray-500">Ce technicien n'a pas encore d'interventions.</p>
                            @can('create', App\Models\Intervention::class)
                                <div class="mt-6">
                                    <a href="{{ route('interventions.create', ['technicien_id' => $technicien->id]) }}" class="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700">
